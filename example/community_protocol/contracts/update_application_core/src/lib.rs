...
mod update_application_core {
    ...
    #[ink(storage)]
    pub struct UpdateApplicationCore {
        ...
        pre_install_community_core: AccountId,
        pre_install_community_sub_token: AccountId,
    ...
    impl UpdateApplicationCore {
        ...
        // pub fn new() -> Self {
        pub fn new(
            pre_install_member_manger:AccountId, 
            pre_install_proposal_manager:AccountId, 
            pre_install_election:AccountId,
            pre_install_community_core:AccountId, 
            pre_install_community_sub_token:AccountId, 
            ...
            Self {
                ...
                pre_install_community_core: pre_install_community_core,
                pre_install_community_sub_token: pre_install_community_sub_token,
                community_list_manager_address: community_list_manager_address,        
            }
        }
        ...
        pub fn configure_pre_install_member_manager(&mut self)-> Result<()>{
            ...
            self.next_pre_software_id.saturating_add(1);
        ...
        pub fn configure_pre_install_proposal_manager(&mut self)-> Result<()>{
            let software_info = SoftwareInfo {
                id: self.next_pre_software_id,
                kind: SoftwareKind::ProposalManager,
                software_type: SoftwareType::PreInstall,
        ...
        #[ink(message)]
        pub fn configure_pre_install_community_core(&mut self)-> Result<()>{
            let software_info = SoftwareInfo {
                id: self.next_pre_software_id,
                kind: SoftwareKind::CommunityCore,
                software_type: SoftwareType::PreInstall,
                name: "Community Core".to_string(),
                description: "PreInstall Community Core.".to_string(),
                contract_address: self.pre_install_community_core,
            };
            self.pre_installed_software_list_with_id.insert(&self.next_pre_software_id, &software_info);
            self.next_pre_software_id.saturating_add(1);

            match self._set_address_for_pre_install_contract("set_application_core_address".to_string(), self.pre_install_community_core, self.env().account_id()){
                Ok(()) => (),
                Err(error) => return Err(error),
            }

            let mut parameter_dsv = String::new();
            parameter_dsv.push_str(&hex::encode(self.pre_install_member_manager));
            parameter_dsv.push_str("$1$");
            parameter_dsv.push_str(&hex::encode(self.pre_install_proposal_manager));
            parameter_dsv.push_str("$1$");
            parameter_dsv.push_str(&hex::encode(self.community_list_manager_address));
            parameter_dsv.push_str("$1$");
            parameter_dsv.push_str(&hex::encode(self.pre_install_community_sub_token));
            // let parameter_dsv = hex::encode(self.pre_install_member_manager) + "$1$"
            //     + &hex::encode(self.pre_install_proposal_manager) + "$1$"
            //     + &hex::encode(self.community_list_manager_address) + "$1$"
            //     + &hex::encode(self.pre_install_community_sub_token);
            let mut instance: DefaultContractRef =
                ink::env::call::FromAccountId::from_account_id(self.pre_install_community_core);
            match instance.extarnal_execute_interface(
                "set_manager_addresses".to_string(),
                parameter_dsv,
                self.env().caller()
            ) {
                Ok(()) => Ok(()),
        ...
        #[ink(message)]
        pub fn configure_pre_install_community_sub_token(&mut self)-> Result<()>{
            let software_info = SoftwareInfo {
                id: self.next_pre_software_id,
                kind: SoftwareKind::CommunitySubToken,
                software_type: SoftwareType::PreInstall,
                name: "Community Sub Token".to_string(),
                description: "PreInstall Community Sub Token.".to_string(),
                contract_address: self.pre_install_community_sub_token,
            };
            self.pre_installed_software_list_with_id.insert(&self.next_pre_software_id, &software_info);
            self.next_pre_software_id.saturating_add(1);

            self._set_address_for_pre_install_contract("set_application_core_address".to_string(), self.pre_install_community_sub_token, self.env().account_id())
        }
        ...
        pub fn install_software(&mut self, proposal_id: u128) -> Result<()> {
                ...
                .get(&software_info.contract_address)
            {
                ...
                None => {
                    ...
                    // todo: update proposal status Executed -> Finished
                    self.next_software_id.saturating_add(1);
                    ink::env::debug_println!("########## application_core:install_software [5] ");
                    Ok(())
                }
            }
        ...
        fn _check_installed_software(&self, taraget_contract_address:AccountId) -> bool {
            if taraget_contract_address == self.pre_install_election ||
                ...
                taraget_contract_address == self.pre_install_community_core ||
                taraget_contract_address == self.pre_install_community_sub_token                 
                {
                    return true;
            }
